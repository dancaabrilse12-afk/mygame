...
io.on('connection', (socket) => {
    ...
    // ── Chat lobby ────────────────────────────────────────────
    socket.on('lobby_chat', ({ code, name, msg }) => {
        if (!msg || !code) return;
        const clean = msg.substring(0, 120);
        io.to(code).emit('lobby_chat', { name, msg: clean });
    });

    // ── Chat in-game ──────────────────────────────────────────
    socket.on('game_chat', ({ code, name, msg }) => {
        if (!msg || !code) return;
        const clean = msg.substring(0, 100);
        io.to(code).emit('game_chat', { name, msg: clean });
    });
